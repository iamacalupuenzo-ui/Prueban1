...
@Component({
  imports: [Button, CLocaterFlotasLogo, Input, PasswordInput],
  template: `
    <main class="login-layout" aria-labelledby="login-title">
      <section class="login-pane">
        <header class="brand"><cs-c-locater-flotas-logo size="lg" /></header>
        <div class="login-content">
          ...
          <form (submit)="signIn($event)" novalidate>
            ...
            <a class="recovery-link" href="#">¿Olvidaste tu contraseña?</a>
            <cs-button variant="primary" size="lg" [fullWidth]="true" [loading]="loading()" (click)="signIn($event)">Ingresar</cs-button>
          </form>
        </div>
        <footer><a href="#">Soporte técnico</a><span aria-hidden="true">·</span><a href="#">Centro de ayuda</a></footer>
      </section>
  ...
  styles: [`
    ...
    .brand { display: flex; align-items: flex-start; min-height: var(--layout-size-3xl); } .brand cs-c-locater-flotas-logo { transform: scale(1.25); transform-origin: left top; }
    ...
    .form-field > label { color: var(--color-text-base-default); font-family: var(--font-family-content); font-size: var(--font-size-content-note); line-height: var(--font-line-height-content-note); font-weight: var(--font-weight-accent); letter-spacing: var(--font-letter-spacing-content); }
...
export class LoginPage {
  ...
  constructor(private readonly router: Router, private readonly auth: MockAuthService) {}
  ...
  protected async signIn(event: Event): Promise<void> {
    ...
    if (result.kind === 'success') { await this.router.navigateByUrl('/dashboard'); return; }
