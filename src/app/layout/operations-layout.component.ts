...
@Component({
  ...
  template: `
    ...
    <div class="operations-shell" [class.is-rail]="layoutState.mode() === 'rail'" [class.mobile-open]="mobileNavOpen()">
      ...
      <aside id="product-navigation" class="product-sidenav" aria-label="Navegación principal" [attr.aria-hidden]="isMobileViewport() && !mobileNavOpen() ? 'true' : null" [attr.inert]="isMobileViewport() && !mobileNavOpen() ? '' : null">
        ...
        <div class="product-menu">
          <cs-menu [groups]="navigationGroups" [mode]="navigationMode()" [activeHref]="activeHref" (navigate)="closeMobileNav()" />
        </div>
      ...
      <main id="main-content" class="operations-main" tabindex="-1">
        <header class="mobile-header">
          <button type="button" class="mobile-menu-trigger" [attr.aria-label]="mobileNavOpen() ? 'Cerrar navegación' : 'Abrir navegación'" [attr.aria-expanded]="mobileNavOpen()" aria-controls="product-navigation" (click)="toggleMobileNav()" csPressScale>
            <cs-icon [name]="mobileNavOpen() ? 'x' : 'menu'" [size]="18" aria-hidden="true" />
          </button>
          <a class="mobile-brand" routerLink="/dashboard" aria-label="C-Locater Flotas, ir al tablero"><cs-c-locater-flotas-logo size="sm" /></a>
        </header>
        <router-outlet />
      </main>
    </div>
  ...
  styles: [`
    ...
    .operations-shell { display: grid; grid-template-columns: var(--layout-sidenav-width-expanded) minmax(0, 1fr); height: 100%; min-height: 0; overflow: hidden; background: var(--elevation-surface-default); transition: grid-template-columns var(--motion-duration-medium) var(--motion-easing-default); }
    .operations-shell.is-rail { grid-template-columns: var(--layout-sidenav-width-collapsed) minmax(0, 1fr); }
    .product-sidenav { display: flex; flex-direction: column; min-height: 0; block-size: 100%; overflow: hidden; border-right: var(--layout-border-thin) solid var(--color-border-divider); background: var(--elevation-surface-default); }
    ...
    .operations-main { min-width: 0; min-height: 0; overflow-y: auto; overscroll-behavior: contain; scrollbar-gutter: stable; background: var(--color-background-neutral-subtlest); }
    .mobile-header, .mobile-backdrop { display: none; }
    @media (max-width: 767px) {
      .operations-shell, .operations-shell.is-rail { grid-template-columns: minmax(0, 1fr); }
      .product-sidenav { position: fixed; z-index: var(--elevation-z-index-dropdown); inset: var(--layout-topnav-height) auto 0 0; width: var(--layout-sidenav-width-expanded); min-height: 0; block-size: calc(100dvh - var(--layout-topnav-height)); border-right: var(--layout-border-thin) solid var(--color-border-divider); box-shadow: var(--elevation-shadow-raised); transform: translateX(-105%); transition: transform var(--motion-duration-medium) var(--motion-easing-default); }
      .mobile-open .product-sidenav { transform: translateX(0); }
      .mobile-backdrop { display: block; position: fixed; z-index: var(--elevation-z-index-dropdown); inset: var(--layout-topnav-height) 0 0; border: 0; background: rgba(0, 0, 0, .36); }
      .product-brand { display: none; }
      .menu-collapse { display: none; }
      .mobile-header { position: sticky; z-index: var(--elevation-z-index-sticky); top: 0; display: flex; align-items: center; gap: var(--layout-gap-lg); min-height: var(--layout-topnav-height); padding-inline: var(--layout-padding-xl); border-bottom: var(--layout-border-thin) solid var(--color-border-divider); background: var(--elevation-surface-default); }
      .mobile-menu-trigger { display: inline-flex; align-items: center; justify-content: center; width: var(--layout-size-md); height: var(--layout-size-md); padding: 0; border: 0; border-radius: var(--radius-md); background: transparent; color: var(--color-icon-base-subtle); cursor: pointer; }
      .mobile-menu-trigger:hover { background: var(--color-interaction-hovered); color: var(--color-text-base-default); }
      .mobile-menu-trigger:focus-visible { outline: none; box-shadow: 0 0 0 var(--layout-border-thick) var(--elevation-surface-default), 0 0 0 var(--layout-border-thicker) var(--color-border-focused); }
      .mobile-brand { display: inline-flex; align-items: center; color: inherit; line-height: 0; }
...
export class OperationsLayoutComponent implements OnDestroy {
  ...
  protected readonly navigationMode = computed(() => this.isMobileViewport() ? 'expanded' : this.layoutState.mode());
  ...
  ngOnDestroy(): void { this.mobileViewportQuery.removeEventListener('change', this.onMobileViewportChange); }
}
